...
export function ledgerText(terms?: CityTerms): string[] {
  if (!terms) return ["Population: no tick yet", "Food: no tick yet", "Materials: no tick yet", "Trade: no tick yet"];
  return [
    ...
    `Materials: ${terms.materials.value.toFixed(1)} = +${terms.materials.produced.toFixed(2)} - ${terms.materials.consumed.toFixed(2)}`,
