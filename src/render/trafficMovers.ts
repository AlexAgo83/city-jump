...
import { laneRank, pickExit } from "../sim/routing";
import { canGo, signalAt, signalCycle, type SignalCycle } from "../sim/signals";
import { laneCentres, ROAD_LIFT, roadType, SIDEWALK_LIFT, SIDEWALK_WIDTH, walkCentres, type LaneCentre } from "../sim/roadTypes";
import {
  armPort,
  junctionTurnPath,
  approachAngle,
...
import type { createVehicleHeadlights } from "./vehicleLights";
import type { createVehicleModels } from "./vehicleModels";
type VehicleModels = ReturnType<typeof createVehicleModels>;
type VehicleHeadlights = ReturnType<typeof createVehicleHeadlights>;
...
export function createTrafficMoverSystem(scene: Scene, graph: RoadGraph, frameDelta: () => number, heightAt: (x: number, z: number) => number, models: VehicleModels, headlights: VehicleHeadlights, state: TrafficMoverState) {
  ...
  function board(mover: RenderMover, segmentId: SegmentId, from: NodeId, entry: Entry, trim: number): void {
    leaveQueue(mover);
    const segment = graph.segment(segmentId);
    const direction = segment.a === from ? 1 : -1;
  ...
  function planAhead(mover: Mover, segment: Segment, direction: 1 | -1): Plan | null {
    return choosePlanAhead(graph, segment, direction, roll(mover), armOf, (node) => ringAt(node).radii.length);
  }
   ...
   */
  function arrive(mover: RenderMover, now: number, occupancy: FrameOccupancy): void {
    const nodeId = mover.direction === 1 ? mover.segment.b : mover.segment.a;
  ...
  function clearMovers(): void {
    ...
    queues.clear();
    queueOf.clear();
    headlights.sync(0, state.lightsOn());
  }
  ...
  function rebuild(dirty?: TerrainBounds): void {
    ...
    for (const [si, seg] of segments.entries()) {
      ...
      const count = scaledTrafficCount(Math.min(4, Math.max(1, Math.floor(seg.length / 80))), state.density);
      for (let i = 0; i < count; i++) {
    ...
    headlights.sync(movers.filter((mover) => !mover.walk).length, state.lightsOn());
  }
  
  /** Faces a mover along a heading, turning towards it rather than snapping onto it. */
  function face(mover: RenderMover, heading: number, dt: number): void {
  ...
  function frameOccupancy(): FrameOccupancy {
    ...
    const ringEntries: { readonly item: RenderMover; readonly key: string; readonly at: number; readonly radius: number }[] = [];
  ...
  function joinQueue(mover: RenderMover): void {
