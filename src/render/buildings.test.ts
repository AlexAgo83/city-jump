...
describe("roof props", () => {
  ...
  it("keeps roof objects on the roof deck, not on parapets or roof huts", () => {
    expect(roofPropY("lot_2x2", -7.25, 11.5)).toBe(10);
    expect(roofPropY("lot_4x4", -15.25, 15.5)).toBe(14);
    expect(roofPropY("lot_1x1", -3.25, 9.5)).toBe(9.5);
    expect(roofPropY("lot_1x1", -0.75, 9.5)).toBeCloseTo(7.58, 2);
  });
});
