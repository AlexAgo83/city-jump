...
import { RoadGraph, type Segment } from "./graph";
import type { JunctionArm } from "./junction";
import { CROSSING_DEPTH, crossingNear } from "./transfers";
import { v3 } from "./vec";
import {
  circularQueueRooms,
  accelerateToward,
...
/**
 * The driving rules, held where they run. These assertions used to live in
 * `render/traffic.test.ts`, which boots a Babylon NullEngine for one renderer case: sixteen
 * simulation cases were loading an engine they never touched, and the simulation layer looked
 * untested from the outside. Nothing here imports Babylon.
 *
 * Where a case turns on a published constant it is written against that constant rather than
 * against the number it currently holds, so retuning a gap or a setback does not rewrite an
 * arithmetic literal whose meaning had gone.
 */
describe("driving rules", () => {
  ...
  it("keeps a circular gap on roundabout rides", () => {
    ...
    expect(rooms.get(a)).toBeCloseTo(1 * 10 - CAR_GAP);
    expect(rooms.get(b)).toBeCloseTo(4 * 10 - CAR_GAP);
    expect(rooms.get(c)).toBeCloseTo((Math.PI * 2 - 5) * 10 - CAR_GAP);
  });
  ...
  it("keeps segment distances direction-aware", () => {
    ...
    expect(stopTarget(90, 80, 1)).toBe(80 - CAR_GAP);
    expect(stopTarget(10, 20, -1)).toBe(20 + CAR_GAP);
    ...
    // What those two numbers mean: the line leaves a bonnet's length clear of the crossing, so a
    // red stops the bumper short of it rather than the car's centre on top of it.
    const room = segment.length - arm.trim - 5;
    const farEdge = crossingNear(arm, room) + CROSSING_DEPTH;
    expect(segment.length - stopLineDistance(segment, 1, 10, 5, arm) - farEdge).toBeCloseTo(CAR_STOP_SETBACK);
    expect(stopLineDistance(segment, -1, 10, 5, arm) - farEdge).toBeCloseTo(CAR_STOP_SETBACK);
