...
4. **Mirror any dimension formula on both sides.** `building_specs()` in `scripts/gen_buildings.py` and `buildingSpec()` in `src/render/buildings.ts` each carry the same height and roof formulas (`6 + ((frontage * 7 + depth * 3) % 5) * 3.5 + min(area, 8)`, roof `2.5` when `area <= 2`). The renderer re-derives them from the model id rather than reading them from the mesh, so changing one without the other puts roof props at the wrong height on every model that formula touches.
...
# Verification
- `npm run test:visual` and look at the shot: the facade faces the road, the building sits inside its plot, and its height reads as plausible next to its neighbours.
...
- `src/render/buildings.ts` -- `loadModel`, `buildingSpec`, `BUILDING_ASSET_VERSION`.
