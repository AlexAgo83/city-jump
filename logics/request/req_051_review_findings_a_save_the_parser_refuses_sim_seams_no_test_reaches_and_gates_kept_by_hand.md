...
- AC7: `.tmp/app.ts.orig` and `.tmp/app.kit.ts` are gone, `scripts/.lights.mjs` is either a linted sibling of the other shot scripts or deleted, the three default-named screenshots at the repository root and the tracked-directory `.DS_Store` files are gone, and `npm run lint` covers every script in `scripts/`.
