...
- The bulldozer's delay was a constant, not slow code: src/render/drawTool.ts set a 1,000 ms demolition delay and routed every road, building and utility demolition through that deferred commit path. Nothing waited on it: there was no demolition animation, `src/render/rubble.ts` only draws the rubble a kaiju leaves, and the target highlight was switched off at the click. So the player clicked, the feedback disappeared, and the road stood for a second.
