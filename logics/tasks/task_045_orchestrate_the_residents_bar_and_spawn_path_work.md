...
# Plan
- [ ] 1. Show the operator the two consequences of a factor of 1000 before building anything: the gate drops from six waves per seed to one, and the wave-1 threat roughly quadruples because waveThreat reads population. Confirm 1000 is still wanted with those on the table.
- [ ] 2. Then the rule itself: a RunRules field defaulting to 1000, validated in readRun, threaded into sim as a parameter, surfaced in the Gameplay row. No module state.
- [ ] 3. Then the gate: pin the harness's own factor where the harness sets it, update the recorded reason in wave.ts, and re-measure the band once -- together with req_042's battery range if that has landed, so the two are not measured separately.
- [ ] 4. Then the landing edges, which are independent of all of the above and cheap.
- [ ] 5. Close with the two loose ends: the unused constant and the drain the return type cannot report.
- [ ] 6. Verify with npm run ci, and npm run scenarios as the band evidence rather than as a pass or fail.
