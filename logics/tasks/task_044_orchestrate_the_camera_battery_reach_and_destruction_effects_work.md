...
# Plan
- [x] 1. Take the camera first: it is the most intrusive of the six and it needs no measurement. Rewrite the reason recorded at src/app/waveLoop.ts:41-44 in the same wave, so no reader meets a comment defending a behaviour that has gone.
- [x] 2. Then the battery range. The flight-time question is already settled -- reach grows, speed does not -- so move the flight time with the range (or stop dividing by it) in the same change, then re-run npm run scenarios once and record the band.
- [x] 3. Then the two HUD moves -- Show FPS into the Camera row, and the counter onto the Wave panel's line -- which are small, independent, and verify by eye.
- [x] 4. Then the destruction effects, which are the only new infrastructure here: explosion and fire as separate lifetimes, instanced like the rubble renderer, with a dispose.
- [x] 5. Close with the toggles, once there are two effects to switch off and a row decided for them.
- [x] 6. Verify with npm run ci, npm run test:visual for the HUD moves, and npm run perf before and after the effects land.
...
- Targeted Playwright checks passed for wave camera, destruction fire reload, and FPS row layout.
...
- Linked backlog item(s): `item_152_stop_a_spawning_kaiju_from_taking_the_camera`, `item_153_double_the_battery_range_and_decide_what_that_does_to_the_missiles`, `item_154_blow_a_building_up_and_set_its_rubble_alight`, `item_155_give_the_two_effects_their_settings_toggles`, `item_156_file_show_fps_with_the_camera`, `item_157_sit_the_fps_counter_beside_the_wave_panel_instead_of_above_it`
- Related request(s): `req_042_let_the_player_keep_the_camera_let_the_batteries_reach_and_show_a_destroyed_building_burning`
