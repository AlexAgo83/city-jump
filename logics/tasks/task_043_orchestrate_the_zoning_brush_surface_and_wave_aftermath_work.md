...
# Plan
- [x] 1. Fix the rubble battery first: it is a live defect, it changes combat balance, and everything else is easier to judge once a flattened barracks stops shooting.
- [x] 2. Take the opening treasury in the same pass, since both move the balance band, then re-run npm run scenarios once and record the band for both.
- [x] 3. Drop the bulldozer's second next -- establish what it was buying before removing it, and retire the deferred-commit guard with it.
- [x] 4. Then the brushes: click-only, which is the same change as giving the left drag back to the camera, and decide what one undo entry covers.
- [x] 5. Then the levelled-city message and the decision about the kaiju, which the rubble fix makes reachable sooner in a run.
- [x] 6. Then evacuation, deciding where the upgrade step lives before wiring evacuate to a new island.
- [x] 7. Then the zoning surface: the tool-choice panel, then the fill itself once the contiguity rule is settled against segment, side and block.
- [x] 8. Close with the brush minimum, folding it into the same pass that gives the radius one source of truth.
- [x] 9. Verify with npm run ci, and with npm run test:e2e on demand since the tool surface and the pointer contract both changed.
...
- 2026-09-05: rtk npm run ci passed after all task_043 slices.
...
- Linked backlog item(s): `item_143_zone_a_contiguous_run_of_lots_from_one_click`, `item_144_put_the_zoning_tool_choice_in_its_own_panel`, `item_145_make_the_brushes_click_only_and_give_the_left_drag_back_to_the_camera`, `item_146_open_the_zone_brush_at_the_minimum_of_its_own_slider`, `item_147_make_evacuating_leave_the_island`, `item_148_say_that_the_city_was_levelled_and_decide_what_happens_to_the_kaiju`, `item_149_stop_a_barracks_in_rubble_from_firing_and_from_holding_its_workers`, `item_150_drop_the_second_the_bulldozer_waits_for_nothing`, `item_151_open_a_run_with_a_treasury_that_can_build_a_city`
- Related request(s): `req_041_what_playing_0_4_0_turned_up_in_the_zoning_tools_the_brush_surface_and_the_wave_s_aftermath`
