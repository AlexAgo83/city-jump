...
const [url = "http://127.0.0.1:5173", out = "lights.png"] =
...
await page.goto(url);
await page.waitForFunction(() => Boolean(window.cityjump), null, {
	timeout: 30_000,
});
const out1 = await page.evaluate(async () => {
	...
	for (const [x, z] of [
		...
		api.road(0, 0, x / 2, z / 2, x, z, "avenue_2lane");
	for (const [x, z] of [
		[900, 0],
		[640, -260],
		[640, 260],
	])
		api.road(640, 0, (640 + x) / 2, z / 2, x, z, "avenue_2lane");
	...
	const masts = scene.meshes.filter(
		(m) => m.name.startsWith("signal_") && !m.name.includes("lamp"),
	);
	const nearRing = masts.filter(
		(m) => Math.hypot(m.position.x - 640, m.position.z) < 200,
	).length;
	...
	const cars = () =>
		scene.meshes.filter((m) => /^traffic_\d+_\d+$/.test(m.name));
	const still = new Map();
	let stopped = 0;
	...
	const observer = scene.onAfterRenderObservable.add(() => {
		...
		for (const car of cars()) {
			...
			const moved = prev
				? Math.hypot(car.position.x - prev.x, car.position.z - prev.z)
				: 1;
			...
			if (r < 14)
				Math.abs(car.position.x) > Math.abs(car.position.z) ? ew++ : ns++;
		}
	...
	return {
		...
		framesWithTrafficInTheJunction: {
			eastWest: insideEastWest,
			northSouth: insideNorthSouth,
			both: bothAtOnce,
		},
	};
});
await page.evaluate(() =>
	window.cityjump.camera(120, Math.PI / 3.4, -Math.PI / 2),
);
