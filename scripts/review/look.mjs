...
const samples = () =>
  page.evaluate(
    () =>
      window.cityjump._scene.postProcessRenderPipelineManager._renderPipelines.look
        .samples,
  );
...
await page.locator("#fx-multisample").uncheck();
await page.waitForTimeout(400);
const stored = await page.evaluate(() => JSON.parse(localStorage.getItem("cityjump.settings") ?? "{}").fxMultisample);
...
check(stored === false, `the choice must be persisted, got ${JSON.stringify(stored)}`);
